...
- **ggplot↔plotly engine toggle (dual-output pattern):** a global status-bar switch (`mod_statusbar`) sets `state$plot_interactive` (default `FALSE` = static ggplot). A toggleable plot is **one ggplot-returning reactive `<id>_gg` feeding `dual_plot("<id>", gg, height)`** (in `mod_qc.R`), which registers a static `plotOutput` + an interactive `plotlyOutput` (`plotly::ggplotly`, `tooltip = "text"`) behind a `uiOutput` container that shows whichever `use_plotly()` selects (spinner moves inside the container). UI placeholder: `.qc_dual_plot(ns("<id>_container"))`. `use_plotly()` = the toggle ON **and** `requireNamespace("plotly")` (it's a `Suggests` — graceful static fallback) **and** `ncol(working) <= .plotly_max_samples` (a sample cap; `ggplotly` serializes every glyph, so interactive is bounded, not unlimited). Builders add a hover **`text` aesthetic only when `interactive = TRUE`** (via `.hover_aes`, so static never warns); the residual ggplot "unknown aesthetic" warning is muffled with `.muffle_unknown_aes`. `validate(need(...))` lives in `<id>_gg` and propagates to whichever output renders. **Excludes** ComplexHeatmap / non-ggplot outputs. Reuse for any future plot page (PCA/DE).
