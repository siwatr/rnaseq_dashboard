...
- **Size factors — a deliberate, decoupled normalization.** DESeq2 median-of-ratios (`estimateSizeFactors`), **unrelated to the CPM/TPM/FPKM assays** (library-size/length normalizations that don't use them) — so *not* a side effect of adding assays; they have their own **"Size factors" Input sub-tab** (`mod_sizefactors`). Rules to hold:
  - **Control set:** endogenous (default) / spike-in / a custom gene set / **all genes (discouraged, ignores the spike-in/exogenous split)**; **estimator `type`** ratio / poscounts / iterate. UI label is "Estimate using:". Size factors are also inspectable on the tab: a **Per-sample** plot and a **Compare** scatter of two methods (x=y line + linear-fit R², consumer-only — nothing written back), and `size_factor` is a plottable **General QC metric**.
  - **How it's computed (the load-bearing rule):** estimate on the control-gene **row-subset**, then the full dds **inherits** the per-sample factors (`sizeFactors(full) <- sizeFactors(dds[control, ])`). This is identical to `controlGenes=` for ratio/poscounts (a gene's geometric mean depends only on its own row) **and** makes `iterate` — which ignores `controlGenes` — honor the set. Per-sample scalars apply to all genes.
  - **Guards:** empty spike-in/custom set → **error** (never silently widen to all genes); `iterate` → **refused** below the design rank (`ncol(model.matrix(design, colData))`); a `normalizationFactors`/tximport-`avgTxLength` object → error (no vector to inherit); non-finite factors → warning.
  - **Config on the dds:** `metadata(dds)$sizefactor_config` (`sizefactor_config()` / `estimate_size_factors(dds, config)`); structural edits **re-estimate under it** (`reestimate_size_factors()`), except externally **"loaded"** factors, which are kept, never re-derived under a guessed method.
  - **Lifecycle:** materialized by default at load/reset (`ensure_size_factors()`: keep an object's own factors as `"loaded"`, else the endogenous default as `"auto"`), so `sizeFactors(dds)` is always present + visible; **DE/PCA/Expression are consumers** (`de_run()` never writes them back).
