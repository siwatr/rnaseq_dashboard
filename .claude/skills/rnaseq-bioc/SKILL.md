...
- **Size factors — a deliberate, decoupled normalization.** They are DESeq2 median-of-ratios (`estimateSizeFactors`), **unrelated to the CPM/TPM/FPKM assays** (library-size/length normalizations that don't use them) — so they are *not* set as a side effect of adding assays; they have their own **"Size factors" Input sub-tab** (`mod_sizefactors`). The **default control set is endogenous** — `estimateSizeFactors(dds, controlGenes = which(feature_class == "endogenous"))` — but the tab lets the user pick **spike-in** or a **custom control-gene set** and the estimator **`type`** (ratio/poscounts/iterate). The choice is a config carried **on the dds** (`metadata(dds)$sizefactor_config`, via `sizefactor_config()`/`estimate_size_factors(dds, config)`), so structural edits **re-estimate under the same config** (`reestimate_size_factors()`). Size factors are **materialized by default at load** (`ensure_size_factors()`: keep an object's own factors, else compute the endogenous default) so `sizeFactors(dds)` is always present and visible; DE/PCA/Expression are **consumers** (`de_run()` never writes them back). (DESeq2's `controlGenes` selects which genes *form* the estimate — a subset used to compute the per-sample factors, applied to all genes.)
