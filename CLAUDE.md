...
2. **QC & filtering** — one **unified** page (a `navset_card_tab` with **Dataset diagnostics** / **Sample QC** / **Filtering** sub-tabs) carrying a **data-type badge**; per-sample (bulk) vs per-cell (single-cell) metrics (library size, detected features, % mito, % spike-in; pure helpers in `qc_helpers.R`, `scater::perCellQCMetrics` with a base-R fallback); `vsn::meanSdPlot` VST plot; sample–sample correlation heatmap (log-counts). Gentle outlier flags (not aggressive auto-drop) for small-n bulk. Feature filtering: always drop all-zero; `edgeR::filterByExpr()` default + manual `rowSums` threshold; **expression filters run on `endogenous` genes only — `spike_in`/`exogenous` are exempt and retained** (consistent with `controlGenes`; keeps spike-ins plottable for dose–response), with an opt-in to also filter them. **Avoid `HTSFilter`** (too slow).
...
- Build the app in **phases** (bulk-first; see roadmap in [rough_design.md](rough_design.md)): P1 = layout + status bar + import (rds + tabular) + export shell *(done)*; P2 = metadata edit, annotation (OrgDb + GTF), normalization, GTF memory minimization + session memory monitor, feature-info annotation UI redesign *(done)*; theming pass (custom palette + dark mode + `thematic`) *(done)*; **P3 = QC + sample/feature filtering (+ ERCC)**, split into sub-PRs — **P3a = per-sample QC metrics + Sample QC tab *(done)***; **P3b = dataset diagnostics (VST `meanSdPlot` + correlation heatmap) ← next**; P3c = filtering (endogenous-only by default); P3d = ERCC dose–response; P3e = UI-polish bundle (ggplot↔plotly engine toggle); P4 = PCA dim-reduction; P5 = DESeq2 + DE plots + heatmap; P6 (later) = single-cell + pseudobulk.
