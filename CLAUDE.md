...
6. **Palette** (`mod_palette.R`) — project-wide colour configuration; the **single source of truth** for discrete colour mappings. The user opts in per metadata column (empty by default), picks a base palette, and **pins** specific levels. The config lives in `state$palette` (a UI preference, untouched by load/reset, no `data_version`) and is resolved by **`palette_helpers.R`**: `palette_discrete(levels, pins, palette)` is the one resolver feeding **both** the ggplot group scales (via the QC builders' `palette =` arg + `.qc_group_scale()`) **and** `qc_annotation_colors(df, config)` for ComplexHeatmap — so plots and heatmap annotations agree. `norm_color()` normalizes hex / R names (`gray50`) / CSS names via `grDevices::col2rgb()`. `thematic` still owns fonts/fg/bg and the *default* discrete scale when a column has no config. **P3g-a** wires the discrete **colData** group only; **P3g-b** adds continuous palettes (`circlize::colorRamp2` + ggplot `gradientn`, `p<pct>` anchors), the rowData/assays/Other groups, and JSON config import/export.
