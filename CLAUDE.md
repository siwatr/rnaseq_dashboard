...
6. **Palette** (`mod_palette.R`) — project-wide colour configuration; the **single source of truth** for discrete colour mappings. The user opts in per metadata column (empty by default), chooses a base palette via a **two-level selector** (type ∈ Qualitative/Sequential/Divergent/Custom → name, e.g. `viridis: magma`, `RColorBrewer: Set2`), and may hand-edit individual level colours (which flips the palette to **Custom**, so the selector never implies the named palette was modified). Per-column config is `list(type, name, colors)` where `colors` is the full level→hex map; it lives in `state$palette` (a UI preference, untouched by load/reset, no `data_version`) and is resolved by **`palette_helpers.R`**: `palette_discrete(levels, colors, type, name, custom)` is the one resolver feeding **both** the ggplot group scales (via the QC builders' `palette =` arg + `.qc_group_scale()`) **and** `qc_annotation_colors(df, config)` for ComplexHeatmap — so plots and heatmap annotations agree. `palette_colors(type, name, n)` enumerates from `viridisLite`/`RColorBrewer` (optional Suggests; graceful Okabe-Ito fallback); `norm_color()` normalizes hex / R names (`gray50`) / CSS names via `grDevices::col2rgb()`. The page also has a **Reference** tab (catalogue swatches by type) and a **Collapse all**. Per-column observers read levels **live** (so a dataset reload doesn't strand them on stale levels) and use `ignoreInit`. `thematic` still owns fonts/fg/bg and the *default* discrete scale when a column has no config. **P3g-a** wires the discrete **colData** group only; **P3g-b** adds continuous palettes (`circlize::colorRamp2` + ggplot `gradientn`, `p<pct>` anchors), the rowData/assays/Other groups, JSON config import/export, and factor management (coerce/reorder levels). The legacy `themer_mode` gallery's Heatmap sub-tab is now superseded by the Reference tab.
