...
2. **QC & filtering** — one **unified** page (a `navset_card_tab` with **Dataset diagnostics** / **Sample QC** / **Sample Correlation** / **Filtering** sub-tabs) carrying a **data-type badge**. *Sample QC*: per-sample (bulk) vs per-cell (single-cell) metrics (library size, detected features, % mito, % spike-in; pure helpers in `qc_helpers.R`, `scater::perCellQCMetrics` with a base-R fallback). *Dataset diagnostics* (pills): `vsn::meanSdPlot` VST plot (VST via `qc_vst()`, which falls back from `DESeq2::vst()` to `varianceStabilizingTransformation()` on small/low-count data), RLE, and per-sample expression density. *Sample Correlation*: a `ComplexHeatmap` correlation heatmap (log-counts; Spearman/Pearson). **Diagnostic plots are less self-explanatory than the Sample QC metrics, so each carries a short "How to read this" guideline directly below it** (`.qc_diag_help`). Gentle outlier flags (not aggressive auto-drop) for small-n bulk. Feature filtering: always drop all-zero; `edgeR::filterByExpr()` default + manual `rowSums` threshold; **expression filters run on `endogenous` genes only — `spike_in`/`exogenous` are exempt and retained** (consistent with `controlGenes`; keeps spike-ins plottable for dose–response), with an opt-in to also filter them. **Avoid `HTSFilter`** (too slow).
...
- Build the app in **phases** (bulk-first; see roadmap in [rough_design.md](rough_design.md)): P1 = layout + status bar + import (rds + tabular) + export shell *(done)*; P2 = metadata edit, annotation (OrgDb + GTF), normalization, GTF memory minimization + session memory monitor, feature-info annotation UI redesign *(done)*; theming pass (custom palette + dark mode + `thematic`) *(done)*; **P3 = QC + sample/feature filtering (+ ERCC)**, split into sub-PRs — **P3a = per-sample QC metrics + Sample QC tab *(done)***; **P3b = dataset diagnostics (VST `meanSdPlot`, RLE, density) + Sample Correlation heatmap *(done)***; **P3c = filtering (endogenous-only by default) ← next**; P3d = ERCC dose–response; P3e = UI-polish bundle (ggplot↔plotly engine toggle); P4 = PCA dim-reduction; P5 = DESeq2 + DE plots + heatmap; P6 (later) = single-cell + pseudobulk.
