...
test_that("dds_content_fingerprint tracks samples, features, and size factors", {
  ...
  # size factors enter the fingerprint
  dds2 <- estimate_size_factors_endogenous(dds)
  expect_false(identical(dds_content_fingerprint(dds), dds_content_fingerprint(dds2)))
...
test_that("adding an assay preserves the DESeq fit + VST; a structural edit stales them", {
  ...
  dds <- estimate_size_factors_endogenous(
    ensure_logcounts(make_mock_dds(n_genes = 60, n_per_group = 3, n_spike = 6, seed = 2)))
